...
drive motor neurons and simple muscle models. Stance is not ended by a clock:
each leg's stance, and with it its paw-contact (cutaneous) input, ends when
that leg's own extensor force falls below a threshold; swing then lasts a
fixed time (\autoref{sec:methods:paced}). A good walking rhythm is one in which extensor
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{fig_init_robustness.png}
  \caption{\textbf{Initialisation robustness of force-triggered stepping.}
    ...
    weight $\mu$: extensor/flexor force correlation per leg, left/right extensor
...
At the two slowest rates the gait does not form. At $\lambda=10^{-5}$ stance
still ends on the force threshold, but with the cutaneous synapse still
