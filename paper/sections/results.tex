...
\paragraph{Three walking speeds, two learning architectures.} We confirmed
two additional timing points bracketing the original one --- a
$\approx31\%$ slower and a modest $\approx6\%$ faster variant, found by
scaling the stance-duration and muscle-fatigue time constants together
rather than moving any one parameter alone --- each cross-validated across
two random seeds. We crossed all three speeds with the two learning
architectures used throughout this paper: a \emph{descending} arm in which
the brainstem-to-rhythm-generator synapse remains plastic (as in the results
...
Figure~\ref{fig:ct_speed_arm_force} shows all six force traces are equally
clean regardless of speed or architecture (corr$(F_E,F_F)=-0.53$ to $-0.85$),
and Figure~\ref{fig:ct_speed_arm_weights} shows the CUT$\to$RG-E weight
converging to the same $\approx63$--$67$\,pA plateau in every case --- the
same initialization-independent set-point reported in
Section~\ref{sec:results:weights}, now shown to hold under closed-loop
timing as well as the fixed clock. A genuinely fast (walk-to-trot-range)
variant of this closed loop was attempted but not yet achieved: several
distinct parameter families remove the timing noise that plagues naive fast
configurations, but the resulting rhythm settles into a fast, perfectly
regular cycle whose left/right phase relationship (alternating vs.\ moving
together) is decided by run-to-run noise rather than by any parameter
tested so far --- reported here as an open problem rather than glossed
over.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\textwidth]{fig_forcetrigger_speed_arm_force_stages.png}
  \caption{\textbf{Closed-loop (force-triggered) stepping is clean across
    three speeds and both learning architectures.} Rows (top to bottom):
    descending arm at medium/slow/fast timing, then sensory arm at
    medium/slow/fast timing; columns: beginning / middle / end of the
    $60$\,s run, as in Figure~\ref{fig:force_l2}. Full stride period noted
    per row. Panels (a)--(r).}
  \label{fig:ct_speed_arm_force}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_forcetrigger_speed_arm_weights_grid.png}
  \caption{\textbf{The learned set-point is the same regardless of speed or
    architecture.} CUT$\to$RG-E weight (solid, left axis) and Ia-E/F$\to$RG
    weights (dashed, right axis) versus time, one panel per speed/arm
    combination from Figure~\ref{fig:ct_speed_arm_force}.}
  \label{fig:ct_speed_arm_weights}
\end{figure}
...
working setting. Figure~\ref{fig:consolidate_desc} shows the descending arm:
...
\begin{figure}[t]
  ...
  \caption{\textbf{Tag-and-capture consolidation, descending arm.} Rows:
    ...
    Figure~\ref{fig:ct_speed_arm_force}. Panels (a)--(i).}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Tag-and-capture consolidation, sensory arm.} Rows: no
    ...
    ($0.15$/$0.20$). Columns as in Figure~\ref{fig:ct_speed_arm_force}.
    Panels (a)--(l).}
...
\paragraph{Scope of this preliminary result.} This subsection covers the
medium and slow walking speeds, full weight-bearing only, both learning
architectures. Two extensions are explicitly not yet included and are
flagged here rather than silently assumed to generalize: a fast
(closed-loop) walking speed, for the phase-locking reason given above; and
toe- and air-stepping (partial and full unloading) under closed-loop timing,
where preliminary checks found the fixed-duration failsafe itself needs
re-tuning for the reduced force ceiling under unloading before a
consolidation gain search is meaningful. Both are separate follow-on
studies. The production-scale confirmatory sweep referenced above is
likewise scoped to the medium and slow speeds, both architectures, at full
weight-bearing only, for the same reason.
