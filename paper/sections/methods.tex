...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{fig_architecture_implemented.png}
  \caption{\textbf{As-implemented architecture (one leg; contralateral is
    the mirror image).} Drawn directly from the verified projection list
    (\autoref{tab:delays}), colour-coded by function: orange = excitatory
    neurons, light/dark orange = static/plastic excitatory projections,
    light blue = inhibitory neurons and projections, red = muscle,
    grey = inputs/afferents (rate-coded muscle$\to$Ia transduction dashed).
    This is the functional instantiation of the schematic in
    Fig.~\ref{fig:schematic}: the descending drive (BS, CUT), the
    reciprocal-inhibition core with the 6:1 InF$\to$RG-E / InE$\to$RG-F
    asymmetry, the Ia proprioceptive interneuron pathway and closed loop,
    the plastic Ia$\to$RG projection (sensory arm), and the motor output
    to the muscle proxies.}
