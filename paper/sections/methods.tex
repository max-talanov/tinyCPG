...
construction (\texttt{-{}-dump-connectivity}). Table~\ref{tab:delays}
...
the plastic projections (CUT$\to$RG-E in both arms, BS$\to$RG in the
descending arm, Ia$\to$RG in the sensory arm) carry lognormal weight
distributions: the table reports their \emph{converged} mean$\pm$s.d.\
(CUT$\to$RG-E and Ia$\to$RG at their learned load set-points, BS$\to$RG
frozen at weak init in the sensory arm). Conduction delays follow the rat
...
\begin{figure}[t]
  ...
  Ia$\to$RG). \textbf{(a)} Weight histograms for the plastic projections
  at initialisation (lognormal). \textbf{(b)} Mean synaptic weight per
  projection (green excitatory, red inhibitory); the asymmetric reciprocal
  inhibition InF$\to$RG-E $=-48$\,pA versus InE$\to$RG-F $=-8$\,pA
  reproduces the Zhang~6:1 ratio. \textbf{(c)} Conduction~+~synaptic delay
  per projection (mean $\pm$ across-connection s.d.), following the rat
  \texttt{length\_velocity} preset with $0.2$\,ms jitter.}
