...
STDP range~\cite{biPoo1998,morrison2007}: $\lambda \in \{1{\times}10^{-4},\,
1{\times}10^{-3},\,1{\times}10^{-2}\}$. We refer to these as
...
\For{$i \in \{0, 1, \dots, 8\}$ \textbf{in parallel} (SLURM array task $i$)}
  ...
  \State $\lambda \gets \{1{\times}10^{-4},\ 1{\times}10^{-3},\ 1{\times}10^{-2}\}[\mathrm{lambda\_idx}]$
